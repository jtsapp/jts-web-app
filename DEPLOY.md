...
> ⚠️ **УСТАРЕЛО.** Описывает старую схему деплоя (Vercel + Neon). Сейчас сайт
> и БД — self-host на VPS через GitLab CI (`compose.yaml` + `compose-app.yaml`,
> своя Postgres вместо Neon). Оставлено как история/референс по LiveKit-агенту
> и TTS-ключам (актуально независимо от того, где хостится сайт).
