...
**Известное ограничение (осознанное):** mood-тег добавляется ТОЛЬКО в `build_instructions` — обычный режим тьютора. Остальные три режима собирают промпт своими билдерами ([agent.py:3272](../../../agent/agent.py)) и блок не получают:

- `build_placement_instructions` (тест уровня) — в его промпте стоит прямой запрет «no markers» и жёсткий счётчик реплик; тег противоречил бы собственным правилам режима;
- `build_debate_instructions` (дебаты) — режим сделан нарочито лёгким;
- `build_scenario_instructions` (ролевые сценарии) — решение клиента: в сценарии агент играет ЧУЖОГО персонажа (пограничник, бариста), а палитра эмоций привязана к тьютору.

Во всех трёх рамка остаётся нейтральной. Кода это не требует: гейт стоит по `self._tutor`, а без блока в промпте тега не будет — значит и публикации.

*Правка от 2026-07-30: в первой редакции здесь стояло «нормальный режим тьютора и сценарии» — фактическая ошибка, сценарии идут отдельным билдером.*
