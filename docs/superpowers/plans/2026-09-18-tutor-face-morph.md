...
> Обновление: план устарел в части таймингов и не знает про успокоение тела
> уходящего (`settleMotion`), неподвижный кадр нового (`is-entering`) и совмещение
> тел в первом кадре движения (`FIRST_FRAME`/`restPose`) — всё это добавлено
> после живой записи и ревью. Источник правды — спека.
