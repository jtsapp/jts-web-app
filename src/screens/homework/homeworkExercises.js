...
/**
 * Задания одной формулировки — в одну карточку.
 *
 * Преподаватель добавляет с урока весь блок разом, и на настоящей работе это
 * восемь десятков заданий. По карточке на задание экран превращался в восемь
 * десятков повторов одной и той же инструкции во весь экран и восьми десятков
 * кнопок «Проверить» — читать невозможно, а решать тем более. В самом уроке
 * этот блок выглядит как одна инструкция и список вопросов под ней; здесь
 * теперь так же.
 *
 * Ключ — тип вопроса ВМЕСТЕ с инструкцией. Одной формулировки мало: у пропуска
 * и у выбора разный разбор, и под одну кнопку «Проверить» их класть нельзя,
 * даже когда текст задания совпал слово в слово. Задания без внятной
 * инструкции (её съедает readableInstruction у вступлений урока) группируются
 * между собой по типу — это лучше, чем каждое своей карточкой.
 *
 * Порядок сохраняется по первому появлению группы: список не должен
 * переставляться от того, что кто-то ответил.
 */
export function exerciseGroups(exercises) {
  const groups = new Map()
  for (const exercise of exercises || []) {
    const title = readableInstruction(exercise.instruction)
    const key = `${exercise.question?.type ?? ''}|${title}`
...
/**
 * Ключ группы для состояния экрана («проверено», «не сохранилось»).
 *
 * Берётся id первого задания, а не сам ключ группировки: тот собран из текста
 * инструкции и в разметке смотрелся бы дико. Задание принадлежит ровно одной
 * группе, поэтому id первого её и опознаёт.
 */
export function groupKey(group) {
  return `hw-${group?.exercises?.[0]?.id ?? group?.key ?? ''}`
}

/** Карточка группы: одна инструкция, список вопросов, одна кнопка. */
export function groupBlock(group) {
  return {
    type: 'practice',
    title: group.title,
    questions: (group.exercises || []).map((e) => cleanQuestion(e.question)),
  }
}
