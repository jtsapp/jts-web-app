 ...
 *
 * 23.09.2026: движок ЗАНЯТИЯ (engineOf) раньше был частью этого решения — STEPS
 * искал разбор, FILE сразу открывал файл во фрейме (spec-lesson-engine-coexistence
 * §6). Но движок это свойство ЗАНЯТИЯ, а не материала: у занятия, заведённого
 * после переключения на FILE по умолчанию, всё ещё можно поставить обычный урок
 * каталога (Запуск/Правка урока картину не проверяют) — и такое занятие тихо
 * показывало файл без разбора, с одной «Section 1» вместо тем урока, хотя файл
 * — самый обычный урок каталога. Разбор ищем всегда, кроме уроков, для которых
 * его заведомо нет: standalone-файл (живой/пробный урок) в каталоге не лежит по
 * определению, а поход за деревом на нём — задержка без единого шанса найти
 * урок. Для настоящего урока каталога результат тот же, что и раньше:
 * `findCatalogLessonId` либо находит урок (STEPS), либо нет (файл своим
 * материалом это и решает, engine здесь заново проверять незачем).
 */
export function shouldResolveCatalogLesson(url) {
