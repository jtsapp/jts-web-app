...
export default function LiveLessonPage({ lessonId, userName, userLevel, token, onNav, onProfile, onBack, onLessonClosed }) {
  ...
  // строка «Материал урока», по которой некуда идти. Сервер снимает стадии с
  ...
  const { connected: liveConnected, sendFocus, sendMirror, sendPresent, sendStepProgress, sendAudio, sendCall, sendWatch } = useLessonLiveSocket(lessonId, token, selfUserId, {
    ...
    onStepProgress: (evt) => {
      ...
      if (!isStaff && evt.senderRole !== 'STUDENT') {
        ...
        //
        // Своего id может не быть вовсе: `userIdFromToken` отдаёт null и на
        // токене без userId (пробный класс), и на нечитаемом payload. Сверять
        // адрес тогда не с чем, и выбрасывать правку — худший из двух исходов:
        // преподаватель видит свою правку и «сохранено», ученик не видит
        // ничего, и не узнаёт об этом ни один из них. Такой клиент применяет
        // адресную правку, как применял до появления адресата.
        // Клиент не смог назвать себя — в токене нет userId (так приходят
        // пробные кабинки). Молча выбросить адресованную ему правку нельзя:
        // преподаватель увидит свой тост и решит, что дошло. Но и применять
        // всё подряд нельзя — это ровно та перезапись ответов у соседей, из-за
        // которой адресат и появился. Поэтому применяем только когда портить
        // некому: в уроке он один.
        // Пока урок не загрузился, состава класса нет — и «он один» читалось бы
        // из пустого списка. В этом окне правку лучше потерять, чем применить
        // чужую: терять тут нечего, повторить преподаватель может.
        const анонимОдин = selfUserId == null && lesson != null && activeParticipants.length <= 1
        ...
        // Перейти за преподавателем и подсветить вопрос — разные вещи, и
        // адресат у них разный. Шаг — поведение класса: преподаватель ведёт
        // занятие для всех, на этом и стоит переключатель «идти за
        // преподавателем» (см. комментарий выше). А указка объясняет
        ...
        if (followTeacher
          ...
          applyTeacherPointer(forMe ? evt : { ...evt, questionId: null })
        } else if (forMe && evt.questionId != null) {
