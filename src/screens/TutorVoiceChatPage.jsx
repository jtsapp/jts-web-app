...
// приходит в начале реплики. Сильная эмоция (радость, злость, сочувствие,
// сарказм — speaks в EMOTIONS) видна всю реплику и ещё это окно; остальные на
// время речи уступают карточке «Говорит» и проявляются, когда тьютор замолкает
// (см. TutorFace). Потом лицо возвращается к «слушаю»: иначе ученик не видит,
// что микрофон снова его.
...
function CallStage({
  ...
  // (карточка «Говорит», если эмоция не из тех, что держатся во время речи).
  let emotion = 'idle'
