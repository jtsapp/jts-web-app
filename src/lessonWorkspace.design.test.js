...
describe('документ урока — поля и выход', () => {
  it('у документа есть свои поля: он рисуется прямо в learn__main, где их нет', () => {
    // Шапка упиралась в сайдбар слева и в кромку окна сверху.
    const doc = css.match(/^\.lw-doc\s*{([^}]+)}/m)[1]
    expect(doc).toMatch(/padding:\s*\d+px/)
  })

  it('вкладки внутри документа не добавляют своих полей поверх общих', () => {
    // Иначе шапка, вкладки и содержимое начинаются с трёх разных отступов.
    const tabs = css.match(/\.lw-doc \.ls__tabs\s*{([^}]+)}/)[1]
    expect(tabs).toMatch(/padding-left:\s*0/)
    expect(tabs).toMatch(/padding-right:\s*0/)
  })

  it('у кнопки выхода из документа есть видимый фокус', () => {
    expect(css).toContain('.lw-doc__back:focus-visible')
  })
})
...
describe('токены доступны там, где рисуется практика', () => {
  it('объявлены на всех корнях, включая задания домашней работы', () => {
    ...
    // `.lv` — оболочка живого урока: её шапка лежит вне `.live`, и токены ей
    // нужны свои (иначе акцентная кнопка выходит бесцветной).
    // `.lw-doc` — документ урока в «Самостоятельно»: когда плеер не осилил урок
    // целиком, он сам себе корень, и без токенов весь урок выходил голым
    // текстом — варианты ответа без фона, карточки без подложки.
    expect(css).toMatch(/\.lv,\s*\.lw,/)
    const roots = css.match(/\.lw,\s*\.live,\s*\.lw-doc,\s*\.hw-exercises\s*\{/)
