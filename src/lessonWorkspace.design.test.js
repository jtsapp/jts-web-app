...
describe('классрум — живой урок: колонки и переходы', () => {
  ...
  it('у ученика две колонки, у преподавателя добавляется маршрут', () => {
    // Макет даёт две колонки — контент 868 и правая 300, — и ученик видит
    // именно их. Маршрут это колонка того, кто ВЕДЁТ занятие: преподаватель
    // прыгает по произвольным шагам, ученика ведут «Назад/Далее» под заданием.
    ...
    const staff = css.match(/\.lw-live-body--route\s*{([^}]+)}/)[1]
