...
/** Сколько глав открыто, когда бэкенд не ответил. Демо-квота живёт в базе, но
 *  эндпоинт книг публичный по смыслу (его открывает и аноним), и «не смогли
 *  спросить» здесь означает «показываем превью», а не «отдаём книгу целиком»:
 *  молчаливый сбой не должен раздавать платный текст. */
export const FALLBACK_PREVIEW_CHAPTERS = 2

/** Имя типа контента на бэкенде. Раньше он назывался BOOK и был переименован
 *  миграцией V204 — старое имя эндпоинт квот больше не разбирает и отвечает
 *  ошибкой, то есть опечатка здесь выглядит как «квота не настроена», а не как
 *  сбой. Держим строку одной константой, чтобы следующее переименование
 *  ловилось тестом, а не жалобой ученика. */
export const BOOKS_CONTENT_TYPE = 'PRACTICE_BOOKS'
 ...
 *
 * @param quota  число глав из ответа бэкенда, либо null («не настроено» или
 *               «не смогли спросить»). Именно ЧИСЛО, а не объект ответа:
 *               вызывающий однажды передал сюда весь объект квоты, и книга
 *               уехала к ученику целиком запертой — `i < {объект}` ложно для
 *               любой главы, а тип объекта проверить было некому. Поэтому всё
 *               нечисловое здесь приравнено к «лимит неизвестен»: демо тогда
 *               получает превью, платящий — книгу целиком, и ошибка вызова
 *               больше не отнимает купленный текст.
 */
export function chapterLimitOf({ authenticated, isDemoAccount, quota }) {
  if (!authenticated) return FALLBACK_PREVIEW_CHAPTERS
  if (Number.isInteger(quota) && quota >= 0) return quota
