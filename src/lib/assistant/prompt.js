...
- Домашнюю работу по английскому целиком за ученика не делай (сочинение, перевод всего текста): помоги разобраться, дай план, проверь его вариант.
...
// Метка, которой модель отвечает на вопрос не по теме (см. RULES). Роут видит
// её в первых токенах, обрывает генерацию и отдаёт OFFTOPIC_REPLY: ответ
// одинаковый и короткий, модель на него не тратится, а лимит ученика — тоже.
export const OFFTOPIC_MARKER = '[[OFFTOPIC]]'

export const OFFTOPIC_REPLY = {
  ru: 'Я помогаю только с английским и с сайтом JTS. Спросите, например, почему ответ не засчитан, какое правило в задании или как работает раздел.',
  kk: 'Мен тек ағылшын тілі мен JTS сайты бойынша көмектесемін. Мысалы, жауап неге қабылданбағанын, тапсырмадағы ережені немесе бөлімнің қалай жұмыс істейтінін сұраңыз.',
  en: 'I can only help with English and with the JTS site. Ask me, for example, why an answer was not accepted, what rule an exercise uses, or how a section works.',
}

/**
 * Начало ответа модели: метка «не по теме», обычный ответ или пока не ясно
 * (текст пуст или ещё может оказаться меткой — «[[OFF…»).
 * @returns {'offtopic'|'normal'|'undecided'}
 */
export function classifyStart(text) {
  const head = String(text ?? '').trimStart()
  if (head.startsWith(OFFTOPIC_MARKER)) return 'offtopic'
  if (OFFTOPIC_MARKER.startsWith(head)) return 'undecided'
  return 'normal'
}
