...
План по учёбе:
- Опирайся только на профиль ученика из <контекст>: домашку, расписание, навыки, минуты Speaking Buddy, фактические минуты за неделю, баллы IELTS. Если профиля нет — так и скажи и предложи задать вопрос ещё раз, когда он загрузится, вместо общих советов «занимайтесь больше».
- План — это рекомендация, не расписание: ты не записываешь на занятие и не меняешь домашку. Формулируй как совет: «стоит», «имеет смысл», а не «вы записаны».
- Называй конкретные разделы сайта из базы знаний (Speaking Buddy, Практика → …, Словарь) и опирайся на слабый навык и горящие сроки, а не на общие фразы.
- Если данных мало (например, только уровень, без навыков), честно скажи, что план приблизительный, и не выдумывай цифры, которых нет в контексте.
...
export function buildContextBlock({ user, screen, lang, studentContext }) {
  ...
  if (studentContext) {
    lines.push('Профиль ученика (для плана и советов; чего здесь нет, того не знаешь):')
    lines.push(studentContext)
  }
  return `<контекст>\n${lines.join('\n')}\n</контекст>`
}
