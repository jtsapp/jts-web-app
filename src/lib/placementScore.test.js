...
import { mergeBank2, simulateSession } from '../practice/placement/engine.generated.js'
import { scorePlacementSession } from './placementScore.js'

// Банк тот же, что отдаётся клиенту.
const raw = JSON.parse(readFileSync(join(process.cwd(), 'public/practice/placement/bank.json'), 'utf8'))
...
const run = (pattern, seed = 7) =>
  simulateSession(bank, raw.manifest, raw.vocab, seed, pattern, 'express').exportJson()

describe('scorePlacementSession', () => {
  it('повторяет вердикт движка по сырым ответам: слабый прогон → A0', () => {
    const session = run('weak')
    const scored = scorePlacementSession(session, bank)
  ...
  it('заявленный уровень не влияет на пересчёт: сервер смотрит только ответы', () => {
    ...
    expect(scorePlacementSession(session, bank).level).toBe('A0')
  })
  ...
  it('незнакомые задания в журнал не идут и уровень не двигают', () => {
    ...
    const scored = scorePlacementSession(withJunk, bank)
    expect(scored.level).toBe(scorePlacementSession(session, bank).level)
  })

  it('пустой журнал не роняет пересчёт', () => {
    const scored = scorePlacementSession({ log: [] }, bank)
