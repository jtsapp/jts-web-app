 ...
 * БД, сеть, 401). Замков в UI больше нет, поэтому null ничего не запирает —
 * просто «Советуем сегодня» показывает первую сцену цепочки.
 */
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!res.ok) return // 503 без БД, 401 — оставляем null
