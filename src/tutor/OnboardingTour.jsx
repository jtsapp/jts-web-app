...
function placePopover(rect, popW, popH) {
  ...
  if (spaceBelow > popH + m) {
    return {
      left: clamp(rect.left + rect.width / 2 - popW / 2, m, vw - popW - m),
      top: rect.bottom + m,
    }
  }
  return {
    left: clamp(rect.left + rect.width / 2 - popW / 2, m, vw - popW - m),
    top: clamp(rect.top - popH - m, m, vh - popH - m),
  }
}
...
export default function OnboardingTour({ steps, onFinish, storageKey }) {
  ...
  useLayoutEffect(() => {
    ...
    measure()
    const settle = setTimeout(measure, 380) // домер после плавного скролла
