...
export const EMOTIONS = {
  ...
  // Ярость — верхняя ступень злости, по дизайн-листу (карточка «Ярость»).
  // Отличается от angry не только цветом: глаза сходятся к переносице (spread),
  // сужаются в щёлки, а рот раскрыт на пол-лица. Пара и оскала тут нет — на
  // таком рте они не читаются, лицо превращается в кашу.
  rage: {
    label: 'Ярость',
    c1: [255, 45, 45],
    c2: [173, 24, 40],
    eyeH: 0.56, eyeW: 1.0, spread: 0.78, bounce: 0.7, speed: 1.9,
    brow: -1, shake: 1, mouth: 'roar',
  },
  ...
  // Злорадство собрано по правилам дизайн-листа: одна ясная идея на лицо,
  // плоские черты, без новых эффектов. Идея — «смотрю искоса и ухмыляюсь»:
  // правый глаз в щёлочку (asym), взгляд вбок (lookX), голова набок (tilt),
  // улыбка перекошена вверх одним углом (skew).
  //
  // Бровей нет намеренно: на листе они есть только у злости и ярости и работают
  // её опознавательным знаком. Прежний вариант нёс бровь -0.35 и потому
  // читался как «сердится и улыбается» — а прищур при 0.85 был почти незаметен.
  gloat: {
    label: 'Злорадствует',
    c1: [255, 178, 102],
    c2: [240, 96, 16],
