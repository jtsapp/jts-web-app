...
describe('normalizeBlock', () => {
  ...
  it('order с рангом за пределами диапазона 0..n-1 отбрасывается', () => {
    // Ранг 3 при трёх словах: набор не образует полную перестановку, значит
    // где-то чип со значением, которого в data-order нет на своём месте.
    expect(
      normalizeBlock({ kind: 'order', prompt: '', words: ['I', 'like', 'coffee'], order: [0, 1, 3], why: '' }, ctx),
    ).toBeNull()
  })

  // Ветка, которую прежний тест обещал заголовком, но не проверял: списки
  // слов и рангов разной длины (страховка на случай, если collectLesson
  // когда-нибудь перестанет строить их из одного и того же списка чипов).
  it('order с разной длиной списков слов и рангов отбрасывается', () => {
    expect(
      normalizeBlock({ kind: 'order', prompt: '', words: ['I', 'like', 'coffee'], order: [0, 1], why: '' }, ctx),
    ).toBeNull()
  })
