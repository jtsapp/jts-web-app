...
- Проект LiveKit один на дев и прод, и пара `LIVEKIT_API_KEY`/`_SECRET` у них
  ОБЩАЯ (строка выше раньше утверждала, что у каждого стенда свой ключ — это
  неверно и стоило одного ложного следа при разборе учёта минут). Значит
  подпись вебхука сходится на любом стенде, куда бы событие ни прилетело, и
  единственный способ не получить `room_finished` — не зарегистрировать URL
  стенда в проекте LiveKit. URL там должно быть ДВА, по одному на стенд; когда
  оба на месте, каждое событие летит на оба, и это безопасно — стенд находит
  комнату только в своей базе, чужое событие уходит в лог без списания.
- Минуты считаются НЕ по жизни комнаты: платное окно — от подключения тьютора
  (`armed_at`, клиент шлёт `armed` на `/api/livekit/session`) до последнего
  пульса вкладки (`last_seen_at`) плюс запас. Поэтому потерянный
  `room_finished` больше не жжёт лимит: он остался страховкой, а не
  единственным путём закрытия сессии. Не выбрасывай его — для комнаты, где
  клиент не успел отметиться, он единственный источник длительности.
