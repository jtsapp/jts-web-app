...
- Вебхук `room_finished` тоже один на проект LiveKit, а стендов два: у каждого
  свой эндпоинт `/api/livekit/webhook`, подписанный своим `LIVEKIT_API_KEY`.
  Не придёт — сессия не закроется, и `closeStaleSessions` спишет её по потолку
  (21 минута при дневном лимите 20), то есть один звонок съест весь день.
