...
MOOD_HINTS: dict[str, str] = {
    ...
    # КАЖДОЙ, и модель читала свой обычный регистр как эмоцию. Имя эмоции должно
    # описывать СОБЫТИЕ, на которое она возникла, а не тон, которым тьютор
    # говорит всегда.
    "anger": "ученик отказывается работать, хамит или сливается с урока",
...
def build_mood_block(tutor: str) -> str:
    ...
    # Отдельная оговорка тьюторам с резким регистром. Декстеру по персоне
    # положены мат и крик в каждой реплике, и без этой строки разбор ошибки
    # приезжает как anger, а не correcting: модель метит СВОЙ ТОН, а не событие.
    # Луне и Спарку строка не нужна — лишний абзац в промпте это лишняя развилка.
    tone = (
        "Разбор ошибки — это correcting, даже если ты орёшь. Слабый ответ, но человек "
        "старается — encourage. anger ставь, только когда злит СОБЫТИЕ, а не когда ты "
        "просто говоришь в своей обычной манере.\n"
        if "anger" in allowed
        else ""
    )
    return (
        ...
        f"Имена:\n{lines}\n"
        f"{tone}"
        "Один и тот же тег подряд не ставь: ничего не изменилось — тега нет.\n"
